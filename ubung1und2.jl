...
# Die Indizes a,b,c sind um +1 verschoben, da Δτ=0 möglich ist,
# was aber in Julia Eintrag Nr.[1] entspricht
function E_pt(n)
    a = (n+1+N)%N +1    # τ+Δτ
    b = (n-1+N)%N +1    # τ-Δτ
    c = (n+N)%N   +1    # τ
    acosh((bigger_corrs[a] + bigger_corrs[b]) / (2*bigger_corrs[c]))
end
...
function error_E_pt(n,J)
    ...
    j = (n-1+N)%N +1        # τ-Δτ
    k = (n+N)%N   +1        # τ
    A = bigger_corrs[i]     # x(τ+Δτ)
    B = bigger_corrs[j]     # x(τ-Δτ)
    C = bigger_corrs[k]     # x(τ)
    a = Jackknife(i, J)     # Δx(τ+Δτ)
    b = Jackknife(j, J)     # Δx(τ-Δτ)
    c = Jackknife(k, J)     # Δx(τ)

    sqrt(abs(a^2/(A^2-4*C^2) + b^2/(B^2-4*C^2) + (c^2*(A+B/2*C))^2 / ((A+B)^2-4*C^2) ))
end
...
image2 = plot!(x_energy_pt, y_energy_pt, yerror = errors_pt,
...
#Lattice Gauge Theory Varnhorst\\ubung2_plot20")
