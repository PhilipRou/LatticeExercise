...
for k = 1:m
    ...
    # Zunächst werden alle räumlichen Links geupdatet (also μ=1)
    ...
    # Nun werden die temporalen Links geupdatet (μ=2)
...
function Delta_staple(new_config, config)
    a = 0
    for mu = 1:2
        for i = 1:N_t
            for j = 1:N_s
                a += -beta * real((new_config[mu][i][j]-config[mu][i][j]) *
                staple_dagger(mu, i, j, config))
            end
        end
    end
    a
...
# Metropolis-Algorithmus:
# Mit dem stimmt auch irgendwas nicht. An der Wirkung Delta_staple liegt es
# wahrscheinich nicht...
raw_configs = [config_1]
for k = 1:m
    test = deepcopy(raw_configs[k])
    ...
    for mu = 1:2
        ...
        P = rand(1)[1]
        Delta = Delta_staple(proposal, test)
        if P < exp(-Delta)
            test = proposal
        end
    end
...
# eine Konfig geupdatet, deren Wirkung gemessen usw.
